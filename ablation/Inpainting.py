...
class Network(nn.Module):
    def __init__(self, rank, posdim, mid_channel): #
        ...
        self.encoding = rff.layers.GaussianEncoding(alpha=1.0, sigma=16.0, input_size=1, encoded_size=posdim//2)
...
if __name__ == '__main__':
    ...
    for rank in ranks:
        ...
        for name in MSI_names:
            ...
            optimizer = optim.Adam([{'params': model.parameters(), 'weight_decay': 0.001}], lr=0.001) #[0.001, 0.001]
                ...
                for iter in range(max_iter):
                    ...
                    if iter % 500 == 0 and iter != 0:
                        ...
                        if ssim >= best_metric[1]:
                            print('SR:',args.visible_ratio, 'name:',name, 'iteration:', iter, 'PSNR', psnr, 'SSIM', ssim, 'NRMSE', nrmse)
        ...
        with open('ablation/r_Inpainting_records.txt', 'a') as file:
            psnr, ssim, nrmse = [metric / len(MSI_names) for metric in average_metrics]
            file.write(f"SR: {args.visible_ratio:.2f}, rank: {rank:d}, PSNR: {psnr:.3f}, SSIM: {ssim:.3f}, NRMSE: {nrmse:.3f}\n")
