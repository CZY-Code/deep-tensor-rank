...
if __name__ == '__main__':
    ...
    # X_GT = torch.from_numpy(X_np).type(dtype)
    ...
    optimizer = optim.Adam([{'params': model.parameters(), 'weight_decay': 0.0001}], lr=0.0001)
    ...
    with tqdm(total=max_iter) as pbar:
        for iter in range(max_iter):
            ...
            loss_2 = 10.0 * torch.norm(grad_.norm(dim=-1) - rand_num**2, 1) #梯度大小固定为1的损失
            loss_3 = 10.0 * torch.norm(torch.exp(-torch.abs(X_Out_off)), 1) #其他点原理0平面的损失
