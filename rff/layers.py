...
class GaussianEncoding(nn.Module):
    ...
    def __init__(self, alpha: Union[Tensor, float]=None,
        ...
        r"""
        Args:
            alpha (Optional[float]): scale
            sigma (Optional[float]): standard deviation of Gaussian b
        ...
        super().__init__()
        if alpha is None:
            self.register_buffer('alpha', torch.ones(encoded_size))
        else:
            self.register_buffer('alpha', torch.ones(encoded_size) * alpha)
    ...
    def forward(self, v: Tensor) -> Tensor:
        ...
        """
        return rff.functional.gaussian_encoding(v, self.alpha, self.b)
